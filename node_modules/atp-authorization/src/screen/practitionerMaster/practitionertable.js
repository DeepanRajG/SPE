...
function PractitionerTable(props) {
  ...
  const getAllPerson = () => {
    ...
    var raw = JSON.stringify({
      ...
      entity: "Person",
      filter:
        `(like(document(first(Person.name)).text,'%%',true))&& Person.roleid[0].roleid!='5' && Person.activestatus==true`,
  ...
  React.useLayoutEffect(() => {
    const getdata = async () => {
      ...
      await dispatch(
        actions.PRACTITIONER_LIST_READ({ page: page, perPage: perPage , tenantid:tenantid,  roleid:roleid })
      );
    };
    getAllPerson();
    getdata();
  ...
  const handleUpdate = async () => {
      if (Valid()) {
        showBackdrop();
        const response = await dispatch(actions.PERSON_UPDATE({
          filter:{
